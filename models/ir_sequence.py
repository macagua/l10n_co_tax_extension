...
class IrSequence(models.Model):
    ...
    _description = "Model that inherit 'ir.sequence' model from odoo core"
    ...
    sequence_dian_type = fields.Selection([
        ...
        default='invoice_computer_generated'
        )
    dian_resolution_ids = fields.One2many(
                'ir.sequence.dian_resolution', 'sequence_id',
...
class IrSequenceDianResolution(models.Model):
    ...
    _description = 'Model to manipulate the sequences DIAN Documents'
    ...
    number_from = fields.Integer(string='Initial number', required=True)
    number_to = fields.Integer(string='Final number', required=True)
    number_next = fields.Integer(string='Next Number', compute='_get_initial_number', store=True)
    number_next_actual = fields.Integer(
        compute='_get_number_next_actual', inverse='_set_number_next_actual',
        string='Current Next Number', required=True, default=1,
        help="Next number of this sequence"
    )
    active_resolution = fields.Boolean(string='Active resolution', required=False)
    sequence_id = fields.Many2one("ir.sequence", string='Main Sequence',
